 ...
 * 时间：2018-3-13
 * 
 * 更新于 2025-12-10 by 创意哥
 * 删除了评论
 * 调理了首页歌单和歌词展示比例，增大歌词字体
 *************************************************/
// 播放器功能配置
var mkPlayer = {
    api: "/api/music", // api地址 - EdgeOne Edge Function
    favoriteApi: "/api/favorite", // 喜欢歌单 API 地址
    loadcount: 20,  // 搜索结果一次加载多少条

    method: "GET",      // 数据传输方式(改为GET，Edge Function 通过 query 参数获取)
...
mkpgb.prototype = {
    ...
    init : function(){  
        ...
        // 添加移动端触摸事件支持
        $(mk.bar + " .mkpgb-dot").on("touchstart", function(e){
            e.preventDefault();    // 取消原有事件的默认动作
        });
        ...
        function barMoveTouch(e) {
            if(!mdown) return;
            var touch = e.originalEvent.touches[0] || e.originalEvent.changedTouches[0];
            if(!touch) return;
            var percent = 0;
            if(touch.clientX < mk.minLength){ 
                percent = 0; 
            }else if(touch.clientX > mk.maxLength){ 
                percent = 1;
            }else{  
                percent = (touch.clientX - mk.minLength) / (mk.maxLength - mk.minLength);
            }
            mk.callback(percent);
            mk.goto(percent);
            return true;
