...
### 技术栈
-----
- **前端**：纯 HTML + CSS + JavaScript（jQuery + layui）
- **后端**：EdgeOne Edge Functions（V8 运行时，无需 PHP/Node.js）
- **数据存储**：EdgeOne KV Storage（喜欢歌单跨设备同步）
- **音乐 API**：公共 Meting API 服务

### 项目结构
-----
```
├── index.html              # 主页面（纯 HTML，无需 PHP）
├── css/                    # 样式文件
├── js/                     # JavaScript 模块
│   ├── player.js           # 播放器配置和核心功能
│   ├── ajax.js             # API 请求模块
│   ├── functions.js        # UI 交互和喜欢歌单功能
│   ├── lyric.js            # 歌词解析模块
│   └── musicList.js        # 播放列表配置
├── images/                 # 图片资源
├── edge-functions/         # EdgeOne Edge Functions
│   └── api/
│       ├── music.js        # 音乐 API（搜索/播放/封面/歌词/歌单）
│       └── favorite.js     # 喜欢 API（KV Storage 读写）
...
2. 连接代码仓库或上传代码
3. **启用 KV Storage**：在控制台 → KV Storage 页面点击"申请开通"
4. **创建命名空间**：创建一个命名空间（如 `music_kv`）
5. **绑定到项目**：在项目设置 → KV Storage 中绑定命名空间，变量名设为 `music_kv`
6. 部署项目

> 注意：如果不配置 KV，喜欢歌单的云端同步功能将使用内存临时存储（重启后数据丢失），但本地播放功能不受影响。

### 与旧版本的区别
-----
| 项目 | 旧版本 (PHP) | 新版本 (EdgeOne) |
|------|-------------|-----------------|
| 入口文件 | index.php | index.html |
| 后端 | PHP (api.php + Meting.php) | Edge Functions (JavaScript) |
| 音乐 API | PHP cURL 调用 | 公共 Meting API |
| 喜欢歌单存储 | 服务端 JSON 文件 (user_data/) | EdgeOne KV Storage |
| 下载功能 | PHP 代理下载 | 浏览器端直接下载 |
| 缓存 | PHP 文件缓存 (cache/) | EdgeOne CDN 缓存 |
| 运行环境 | PHP 5.4+ 服务器 | EdgeOne Pages（无需服务器） |
...
### 更新日志
-----
#### v3.0 `2026/5/6` - EdgeOne 迁移版
- 技术栈从 PHP 迁移为纯 HTML + CSS + JavaScript
- 后端从 PHP (api.php + Meting.php) 迁移为 EdgeOne Edge Functions
- 音乐 API 使用公共 Meting API 服务
- 喜欢歌单存储从服务端 JSON 文件迁移为 EdgeOne KV Storage
- 下载功能改为浏览器端直接下载，无需后端代理
- 移除了 PHP 依赖（api.php, Meting.php, Download.php）
- 移除了服务端缓存机制（cache/ 目录），由 EdgeOne CDN 处理
- 前端 API 调用参数从 `types=` 改为 `action=`
...
#### 播放器DIY教程
除了自定义播放列表，本播放器还支持一些 DIY 设定，比如修改 API 地址、修改搜索框的默认搜索内容等。具体请打开 `js/player.js` 查看 `mkPlayer` 配置对象。

#### API 说明
- **音乐 API**：`/api/music?action=search|url|pic|lyric|playlist|userlist`
- **喜欢 API**：`/api/favorite?action=get|save|incremental_sync`
